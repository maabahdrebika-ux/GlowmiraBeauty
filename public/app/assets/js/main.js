// Item name: Glowmira - Beauty Salon & Cosmetic HTML Template
// Creator: Khanh Vu
...
const onOpenQuickViewModal = () => {
    $(".product-qv").on("click", function(e) {
        ...
        updateQuickViewModal(productId, productName, productCategory, productPrice, productImage);
...
const updateQuickViewModal = (productId, productName, productCategory, productPrice, productImage) => {
    // Update category
    ...
    // Update price
    $('#modal-product-price').text('$' + parseFloat(productPrice).toFixed(2));

    // Update product image
    $('#modal-product-image').attr('src', productImage).attr('alt', productName);

    // Update product images in slider
    const sliderWrapper = $('.product-detail__slide-one .slider-wrapper');
    sliderWrapper.empty(); // Clear existing slides
    ...
    // Add some placeholder additional images for the slider
    const additionalImages = [
        productImage,
        '{{ asset("app/assets/images/product/2.png") }}',
        '{{ asset("app/assets/images/product/3.png") }}',
        '{{ asset("app/assets/images/product/4.png") }}'
    ];

    // Add additional images (excluding the main one if it's already added)
    additionalImages.slice(1).forEach(image => {
        sliderWrapper.append(`
            <div class="slider-item">
    ...
    // Update additional product information if available
    // These can be extended to include more product data
    console.log('Modal updated with product data:', {
        id: productId,
        name: productName,
